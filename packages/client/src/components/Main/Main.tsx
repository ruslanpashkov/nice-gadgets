...
export const Main: FC = () => {
  ...
  const loadProducts = useCallback(async () => {
    try {
      ...
      const [newest, discount, counts] = await Promise.all([
        getNewProducts(),
        getDiscountProducts(),
        getProductsCount(),
      ]);
  ...
  return (
    <div className="Main">
      ...
      <Recommendations title="Brand new models" products={newProducts} />

      <Categories totals={totals} />

      <Recommendations title="Hot prices" products={discountProducts} />
    </div>
  );
};
